...
final class MDDLinkTest extends FlatSpec with Matchers with Inspectors {
  ...
  it should "filter the same as MDD" in {
    ...
    val d = 5
    val k = 5
    val mdd = MDDGenerator(d, k, .28, 0, rand).reduce
    val gen = new ExtensionGenerator(new ParameterManager())
    var mddr = gen.cspomMDDtoCspfjMDD(mdd).reduce()
    var mddl = gen.cspomMDDtoMDDLink(mdd).reduce()

    mddr.edges(-2) shouldBe mdd.edges
    println(mddr.edges(-1), mddl.edges(-1))
