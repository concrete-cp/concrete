...
public class ConcreteAC3ConstraintTest {
	...
	public void setUp() throws IOException, SQLException, URISyntaxException,
		...
		concrete = new Concrete(new String[] { "-parameter",
				"mgac.addConstraints=BIN", "-filter",
				"cspfj.filter.AC3Constraint", "" });
	...
	public void crosswordm1() throws FileNotFoundException, ParseException,
		...
		assertEquals(1, problem("crossword-m1-debug-05-01.xml", false));
		assertEquals(48, problem("crossword-m1-debug-05-01.xml", true));
	...
	private int problem(String name, boolean all) throws FileNotFoundException,
		...
		cspom.CSPOM cspomProblem = cspom.CSPOM.load(getClass().getResource(
				"../" + name));
		Problem problem = Problem.load(new CspOM(cspomProblem, -1, true), -1);
