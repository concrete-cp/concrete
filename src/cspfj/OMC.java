...
public final class OMC extends AbstractSolver {
	...
	public OMC(Problem prob, ResultHandler resultHandler) {
	...
	private Variable findBest(final TieManager tieManager) {
		Variable bestVariable = null;

		tieManager.clear();

		for (Variable v : problem.getVariables()) {
			if (v.getCurrentConflicts() <= 0) {
				continue;
			}

			final int vid = v.getId();

			if (tieManager.newValue(vid, flipCounter[vid])) {
				bestVariable = v;
			}
	...
	private int bestWalk(final TieManager tieManager) {
		...
		final Variable bestVariable = findBest(tieManager);
		...
		final int bestIndex = bestVariable.bestImprovment();

		int improvment = bestVariable.getImprovment(bestIndex);
	...
	public void minConflicts() throws MaxBacktracksExceededException,
		...
		while (nbConflicts > 0) {
			...
			if (FINE) {
				logger.fine(nbConflicts + " conflicts, " + getNbBacktracks()
						+ "/" + getMaxBacktracks() + ")");
			}
