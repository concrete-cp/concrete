...
public class RBGenerator {
	...
	public CSPOM generate() throws FailedGenerationException {
		...
		for (int i = 0; i < activeConstraints.length; i++) {
			...
			cspom.ctr(buildExplicitConstraint(involvedVariables, tightnessMode, tightness,
	...
	private long computeNbUnallowedTuplesFrom(IntVariable[] variables, double tightness) {
		long cpt = 1;
	...
	private CSPOMConstraint buildExplicitConstraint(IntVariable[] variables,
		...
		return new CSPOMConstraint("extension", variables, CSPOMConstraint.param("relation",
				matrix.e).param("init", matrix.sup));
	}
