...
/**
 * This class allows generating integer random lists. A random list is formed of
 * fixed-size tuples, the values of which range from 0 to a limit given by the
 * user. When generating random lists, it is possible to indicate:
 * <ul>
...
public abstract class RandomListGenerator {
    ...
    /**
     * The number of values for each element of the tuples of the generated
     * lists. The array index corresponds to the order of the elements of the
     * tuples. The array value gives the number of values for the corresponding
     * element. For instance, if nbValues[i] = n then it means that the ith
     * element of each tuple can belong to the propagationSet
     * <code> {0,1,...,n-1} </code>.
     */
    ...
    /**
     * The type of the generated lists which can be UNSTRUCTURED, CONNECTED or
     * BALANCED.
     */
    protected Structure type;

    /**
     * Indicates if the same tuple can occur several times in the generated
     * lists.
     */
    ...
    /**
     * A particular tuple, which if not <code> null </code>, must or must not
     * belong to the generated lists.
     */
    protected int[] fixedTuple;

    /**
     * Indicates if the fixed tuple, if not <code> null </code>, must or must
     * not belong to the generated lists.
     */
     ...
     * 
     * @param type
     *            the type of the generated lists which can be UNSTRUCTURED,
     *            CONNECTED or BALANCED
     * @param tupleRepetition
     *            indicates if the same tuple can occur several times in the
     *            generated lists
     * @param valueRepetition
     *            indicates if the same value can occur several times in a
     *            generated tuple
     * @param fixedTuple
     *            a particular tuple, which if not <code> null </code>, must or
     *            must not belong to the generated lists
     * @param requiredFixedTuple
     *            indicates if the fixed tuple, if not <code> null </code>, must
     *            or must not belong to the generated lists
     */
    public void setParameters(Structure type, boolean tupleRepetition,
        ...
        if (computeNbDistinctTuples() == 0)
            throw new IllegalArgumentException(
                    "The number of distinct tuples is equal to 0");
     ...
     * 
     * @param n
     *            the number of elements that can be selected
     * @param p
    ...
    public static double computeNbCombinationsFrom(int[] t) {
        ...
        return computeNbCombinationsFrom(tmp, 0, 0); // CHANGER POUR PLUS
                                                     // EFFICACE
    }

    /**
     * Reurns the number of distinct tuples (hence, we do not take into account
     * tuple repetition).
     */
    protected double computeNbDistinctTuples() {
        return (valueRepetition ? computeNbArrangementsFrom(nbValues)
    ...
    // double d = (valueRepetition ? computeNbArrangementsFrom(nbValues) :
    // computeNbCombinationsFrom(nbValues));
    ...
    public void saveElements() {
        ...
        String name = "List_" + nbValues + "_" + tupleLength + "_" + seed + "_"
                + getSelectionDescription() + "_" + getTypeDescription() + "_"
                + tupleRepetition + "_" + valueRepetition
                + (fixedTuple != null ? "_sat" : "") + ".txt";
    ...
    public void displayTuples() {
        double max = (tupleRepetition ? -1
                : (valueRepetition ? computeNbArrangementsFrom(nbValues)
                        : computeNbCombinationsFrom(nbValues)));

        System.out.println("There are " + tuples.length + " elements "
                + (max != -1 ? "/  " + max + " elements" : ""));
        for (int i = 0; i < tuples.length; i++) {
            System.out.print("(");
            for (int j = 0; j < tuples[i].length; j++) {
