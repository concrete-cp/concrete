...
public abstract class BitVector implements Cloneable {
  ...
  public static BitVector intBv(int lb, int ub) {
    final BitVector bv = newBitVector(ub + 1);
    bv.setFrom(lb);
    return bv;
  }

  public static BitVector intBvH(int lb, int ub, int hole) {
    final BitVector bv = intBv(lb, ub);
    bv.clear(hole);
    return bv;
  }

  public static BitVector fullBV(int size) {
    final BitVector bv = BitVector.newBitVector(size);
    bv.fill(true);
    return bv;
  }
  ...
  public abstract boolean setFrom(final int from);
