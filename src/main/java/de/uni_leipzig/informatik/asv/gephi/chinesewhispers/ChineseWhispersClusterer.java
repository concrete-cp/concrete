...
public class ChineseWhispersClusterer implements Clusterer, LongTask {
	...
	public boolean getUseVisibleGraph() {
		return useVisibleGraph;
	}

	public void setUseVisibleGraph(boolean useVisibleGraph) {
		this.useVisibleGraph = useVisibleGraph;
	}
	...
	public enum Propagation {
		...
		public String toString() {
			return NbBundle.getMessage(ChineseWhispersClusterer.class,
					"ChineseWhispersClusterer.propagation." + this.name());
		}
	}
	...
	public enum Unconnected {
		...
		public String toString() {
			return NbBundle.getMessage(ChineseWhispersClusterer.class,
					"ChineseWhispersClusterer.unconnected." + this.name());
		}
	}
	...
	public void execute(GraphModel gm) {
		...
		for (int i = 0; i < iterations; i++) {
			...
			Map<Node, Integer> classesBuffer = new HashMap<Node, Integer>(
					classes);
			for (Node node : inputNodes) {
				...
				for (Edge edge : graph.getEdges(node)) {
					...
					case TOP:
					case VOTE:
						impact = edge.getWeight();
						break;
					case DIST:
						impact = edge.getWeight() / graph.getDegree(otherNode);
						break;
					case DIST_LOG:
						impact = edge.getWeight()
								/ Math.log(1 + graph.getDegree(otherNode));
						break;
					}
					if (rankedClasses.get(neighbourClass) != null)
						rankedClasses.put(neighbourClass,
								rankedClasses.get(neighbourClass) + impact);
					else
				...
				for (Map.Entry<Integer, Double> entry : rankedClasses
						.entrySet()) {
					sum += entry.getValue();
				...
				if (propagation != Propagation.VOTE
						|| highestWeight / sum > propagationVoteValue) {
		...
		for (Cluster cluster : result) {
			((ChineseWhispersClusterImpl) cluster).setName("Cluster #" + i++);
		}
