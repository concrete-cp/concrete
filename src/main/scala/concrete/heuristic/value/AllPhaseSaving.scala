...
final class AllPhaseSaving(heuristic: ValueHeuristic) extends ValueHeuristic {

  private val previous = new HashMap[Variable, ListBuffer[Int]]
  ...
  override def selectIndex(variable: Variable, domain: Domain) = {
    val order = previous.getOrElseUpdate(variable, new ListBuffer)

    order.find(domain.present)
      .getOrElse {
