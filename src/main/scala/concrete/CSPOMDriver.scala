...
object CSPOMDriver {
  ...
  def sumProd(coefVar: (Int, SimpleExpression[Int])*)(implicit problem: CSPOM): SimpleExpression[Int] = {
    val (coefs, vars) = coefVar.toSeq.unzip
  ...
  def linearReif(mode: String, constant: Int, scalar: (Int, SimpleExpression[Int])*)(implicit problem: CSPOM): SimpleExpression[Boolean] = {
    val (coefs, vars) = scalar.toSeq.unzip
...
final class JCSPOMDriver extends CSPOM {
  ...
  def lt(v1: SimpleExpression[Int], v2: SimpleExpression[Int]): SimpleExpression[Boolean] = v1 < v2

  def less(v1: SimpleExpression[Int], v2: SimpleExpression[Int]): SimpleExpression[Int] = v1 - v2

  def neq(e1: CSPOMExpression[Any], e2: CSPOMExpression[Any]): SimpleExpression[Boolean] = e1 !== e2

  @varargs
  def allDifferent(v: IntVariable*): CSPOMConstraint[Boolean] = CSPOMDriver.allDifferent(v: _*)

  def abs(v: SimpleExpression[Int]): SimpleExpression[Int] = CSPOMDriver.abs(v)
