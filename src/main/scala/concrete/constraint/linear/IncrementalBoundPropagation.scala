...
trait IncrementalBoundPropagation extends Linear with StatefulConstraint[(Array[Domain], Interval, BitVector, Int)] {
  ...
  def initData(ps: ProblemState) = {
    ...
    val maxI = doms.head.span.size * math.abs(factors.head)
    val vars = BitVector.filled(arity).filter(doms(_).size > 1)
  ...
  final def processUB(
    ...
    if (f.ub <= 0) {
      /** Entailed */
      PFiltered(hasChanged, true, f, vars, -1)
    } else if (f.lb > 0) {
      PContradiction
    } else if (i < 0) {
      ...
      /** Short-circuit */
      PFiltered(hasChanged, false, f, vars, math.max(max, is(i)))
    } else {
      val dom = doms(i)
      ...
      if (newDom eq dom) {
        processUB(vars.nextSetBit(i + 1), vars, doms, f, factors, ps, hasChanged, max)
//      } else if (newDom.isEmpty) {
//        throw new IllegalStateException
//        PContradiction
      } else {

        val newMax = math.max(max, newDom.span.size * math.abs(fact))
