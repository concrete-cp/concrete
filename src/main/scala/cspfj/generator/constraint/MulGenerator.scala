...
final class MulGenerator(problem: Problem) extends AbstractGenerator(problem) {
  ...
  def generate(constraint: CSPOMConstraint) = {
    ...
    if (Seq(result, v0, v1) filter (_.getDomain == null) match {
      ...
      case Seq(v) if v == v0 => {
        v.setDomain(new BitVectorDomain(generateDomain(result, v1): _*))
        true
      }
      case Seq(v) if v == v1 => {
        v.setDomain(new BitVectorDomain(generateDomain(result, v0): _*))
        true
      }
      case _ => false
  ...
  private def generateDomain(result: Variable, variable: Variable) = {
    AbstractGenerator.makeDomain(
      for {
        i <- result.getDomain.allValues
        j <- variable.getDomain.allValues
        if (i % j == 0)
      } yield i / j)
  }
