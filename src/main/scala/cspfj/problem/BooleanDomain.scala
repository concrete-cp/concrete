...
class BooleanDomain extends Domain with Cloneable {
    ...
    def first = status.first


    def size = status.size
    
def last = status.last

    
    def nextIndex(i: Int) = status.next(i)
    
    def prevIndex(i: Int) = status.prev(i)
   ...
   def present(i: Int) = status.present(i)
    ...
    def clone = super.clone.asInstanceOf[BooleanDomain]
